...
parser.add_argument('--train_way',default=-1,type=int)
...
directory = './results/lfw/base{}shot'.format(args.shot)
...
if(n_labels_for_training<1):
    n_labels_for_training = 5
n_labels= 5
...
def train(epoch):
    ...
    for batch_idx in range(1000):
        ...
        z_proto_list = []
        #prototype z
        for pt in range(n_labels_for_training):
        ...
        for inpi in range(len(inp)):
            ...
            softmaxProb = F.softmax(-distVect).view(n_labels_for_training, 1)

            val,pos =torch.max(softmaxProb,0)

            problabelRounded = torch.zeros(1,n_labels_for_training)
            problabelRounded[0,pos.data.item()] = 1

            target = actLabelList.view(n_labels_for_training,1)
            ...
            cellsum = 0
            for lp in range(n_labels_for_training):
                if(labelprod[lp,0].data.item()>0):
                    cellsum = cellsum + (-1*torch.log(labelprod[lp,0]))

            logloss =  cellsum/n_labels_for_training

            if( torch.sum(problabelRounded == actLabelList) == n_labels_for_training):
                corCount = corCount + 1
...
def testval(epoch):
    ...
    print('Val============================> Epoch: {} Average Val loss: {:.20f}'.format(
        epoch, test_loss))
...
path =  './results/lfw/base{}shot/lfw_first_{}.torch'.format(args.shot,epoch)
...
path =  './results/lfw/base{}shot/lfw_last_{}.torch'.format(args.shot,epoch)
torch.save(modelresnet50.state_dict(), path)
