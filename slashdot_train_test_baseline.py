...
directory = './results/slashdot/base{}shot'.format(args.shot)
if not os.path.exists(directory):
    os.makedirs(directory)

import pickle

tensorFeat=pickle.load(open("./data/slashdot/slashdotFeat.pkl","rb"))
tensorLabel=pickle.load(open("./data/slashdot/slashdotLabel.pkl","rb"))
...
class FewShotEpisodeGeneratorLabel(nn.Module):
    ...
    def getEpisode(self,mode):
        ...
        for fg in range(len(labelarr)):
            if(labelarr[fg]==2 or labelarr[fg]==18):
                labelarr[fg] = labelarr[fg] + 1
...
# Phase 1 - train on first 10 labels and test on the remaining 12
# Phase 2 - train on last 12 labels and test on the first 10

print("Training will be done twice")
print("Phase 1 - train from scratch on first 10 labels and test on the remaining 12")
print("Phase 2 - train from scratch on last 12 labels and test on the first 10")




# Train on first 10 label and test on rest

print("Phase 1 starts----------Train on first 10 label and test on rest---------")
...
path =  './results/slashdot/base{}shot/slashdot_first_{}.torch'.format(args.shot,epoch)
torch.save(model.state_dict(), path)

# Train on last 12 labels and test on the first 10 labels

print("Phase 2 starts----------Train on last 12 labels and test on rest---------")
...
path =  './results/slashdot/base{}shot/slashdot_last_{}.torch'.format(args.shot,epoch)
torch.save(model.state_dict(), path)
